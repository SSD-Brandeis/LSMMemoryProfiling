...
SELECTIVITY = 0
SIZE_RATIO = 10
NUM_PAGES = [512, 1_024, 2_048, 4_096]
PAGE_SIZES = [2_048, 4_096, 8_192, 16384]
    ...
    for buffer in BUFFERS_TO_PLOT:
        for pages in NUM_PAGES:
            for entry_size in ENTRY_SIZES:
                ...
                # )
                page_size = buf_bytes // pages
                if entry_size > page_size or page_size % entry_size != 0:
                    data.append(
                        {
                            "buffer": buffer,
                            "page_size": page_size,
                    ...
                    continue
                entries_per_page = buf_bytes // (entry_size * pages)
                log_dir = (
                    STATS_DIR
                ...
                # ) / (1024 * 1024)
                total_data_size_mb = (buf_bytes - total_data_size_avg) / (1024 * 1024)
                data.append(
                    {
                        "buffer": buffer,
