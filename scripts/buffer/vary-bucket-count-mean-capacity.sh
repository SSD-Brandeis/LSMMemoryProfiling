...
TAG=vary-bucket-count-small-exp
ENTRY_SIZE=128
...
########################################
# Non-hash baselines — run once (independent of bucket count)
########################################
mkdir -p \
    "$BASE_DIR/skiplist" \
...
cd "$REPO_ROOT"
echo "Done."
echo "All vary-bucket-count-small experiments finished."
...
MESSAGE="vary-bucket-count-small Experiments Completed on ${HOSTNAME}"
