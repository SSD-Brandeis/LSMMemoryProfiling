...
# ENFORCEMENT: Move into the results directory BEFORE any generation occurs.
# This ensures workload.txt and workload.specs.json are contained within the result folder.
cd "$BASE_EXP_DIR"
...
# Check only within the current result directory for the spec file
if [ -f "workload.specs.json" ]; then
    SPEC_PATH="workload.specs.json"
else
    echo "Error: workload.specs.json not found in $(pwd). Ensure it is in the results folder."
    exit 1
fi
...
# Since we are already inside BASE_EXP_DIR, workload.txt is the master copy.
MASTER_WORKLOAD="$(pwd)/$WORKLOAD_TXT"
...
# Clean up copies in subfolders, but leave the master copy in BASE_EXP_DIR
find . -mindepth 2 -name "$WORKLOAD_TXT" -delete

echo "Cleanup complete. Master workload preserved in $(pwd)."