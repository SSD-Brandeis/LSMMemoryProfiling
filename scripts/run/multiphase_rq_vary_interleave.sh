...
TAG="multiphase_rq_interleave"
RUN_PREALLOCATED=1
RUN_DYNAMIC_VECTORS=0  # Set to 1 to enable dynamic vectors, 0 to skip
...
# Combined Selectivity Array
SELECTIVITIES=( 0.1 0.01 0.001 0.0001  0.00001  0.000001)
...
# Rebuild once before all experiments
bash ./scripts/rebuild.sh

# ==============================================================================
# BEGIN SELECTIVITY LOOP
# ==============================================================================
for SELECTIVITY in "${SELECTIVITIES[@]}"; do

    echo "================================================================================"
    echo "Starting experiments for SELECTIVITY: $SELECTIVITY"
    echo "================================================================================"
    ...
    # Source of truth for all buffer runs in this selectivity iteration
    ...
    for mem in "${!BUFFER_IMPLEMENTATIONS[@]}"; do 
        ...
        # --- BRANCH 2: Vector Types (Dynamic & Preallocated) ---
            ...
            # 1. DYNAMIC (Uses -A 0) - Optional based on flag
            if [ "$RUN_DYNAMIC_VECTORS" -eq 1 ]; then
    ...
    echo "------------------------------------------------"
    echo "Selectivity $SELECTIVITY experiments finished. Cleaning up workload files..."

    # Remove the master workload file in the current directory (BASE_EXP_DIR)
    rm -f "$WORKLOAD_TXT"
    find . -maxdepth 2 -name "$WORKLOAD_TXT" -delete
    echo "Cleanup complete for $SELECTIVITY."

    # Return to the project root for the next selectivity iteration
    cd "$PROJECT_ROOT"

done

echo "================================================================================"
echo "All selectivity experiments completed successfully."
echo "================================================================================"