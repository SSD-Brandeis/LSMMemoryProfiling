...
class Logger(object):
    def __init__(self, filename):
        ...
        # Save log inside the script-named folder
...
EXP_DIR = ROOT_DIR / "diskbased_vary_delete"
...
def process_run_metrics(args):
    ...
    if not content:
        print(f"[WARNING] No log content found for {impl} in {path}")
        return impl, metrics
...
def plot_throughput():
    for s in ["linear", "log"]:
        generate_lineplot_for_metric("throughput", "throughput (k ops)", "throughput", divisor=1000, scale=s)

def plot_overall_datamovement():
    for s in ["linear", "log"]:
        generate_lineplot_for_metric("data_movement_gb", "data movement (GB)", "overall-datamovement", divisor=1, scale=s)

def plot_flush_counts():
    for s in ["linear", "log"]:
        generate_lineplot_for_metric("flush_count", "flush count (k)", "flush-counts", divisor=1000, scale=s)

def plot_compaction_counts():
    for s in ["linear", "log"]:
        generate_lineplot_for_metric("compaction_count", "compaction count (k)", "compaction-counts", divisor=1000, scale=s)
