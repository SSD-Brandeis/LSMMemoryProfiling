...
TAG = f"filepickingpolicy-{POLICY}"

CURR_DIR = Path.cwd()
PROJECT_ROOT = CURR_DIR.parent.parent
ROOT_DIR = PROJECT_ROOT / ".vstats"
OUT_DIR = PROJECT_ROOT / TAG
os.makedirs(OUT_DIR, exist_ok=True)
EXP_DIR = ROOT_DIR / "filepickingpolicy-exp" / POLICY

# Same workload/run setup as diskbased-exp, but this workload has no
# --phase_boundaries (see run.log/db_env.h), so every run below produces a
# single aggregate "phase" rather than the 3 phases the original
# plot_disk-based_exp.py script was written against.
implementations = [
    "vector-preallocated",
...
def plot_PQ_latencies():
    ...
    ax.set_xticks([1, 2, 3, 4, 5, 6, 7, 8, 9], ["vec", "uvec", "skip", "iskip", "hlink", "hskip", "hvec", "art", "btree"], rotation=90)
    ax.set_yticks([0, 10, 20], ["0", "10", "20"])
    ax.set_ylabel("PQ latency (ms)", labelpad=-1)  # $\\mu$      , labelpad=-1
    # ax.set_xlabel("buffer", labelpad=-1)
    ax.set_ylim(0, None)

    output_file = OUT_DIR / "disk-based-insert-latency-pq.pdf"
    plt.savefig(output_file, bbox_inches="tight", pad_inches=0.02)
    print(f"Saved plot to {output_file}")
...
def plot_RQ_latencies():
    ...
    output_file = OUT_DIR / "disk-based-insert-latency-rq.pdf"
    plt.savefig(output_file, bbox_inches="tight", pad_inches=0.02)
    print(f"Saved plot to {output_file}")
    ...
    legend_file = OUT_DIR / "disk-based-latency-legend-violin.pdf"
    fig_legend.savefig(legend_file, bbox_inches="tight", pad_inches=0.02)
    plt.close(fig_legend)
    print(f"Saved legend to {legend_file}")
...
def plot_insert_latencies_violin():
    ...
    # --- Save Plot ---
    output_file = OUT_DIR / "disk-based-insert-latency-violin.pdf"
    plt.savefig(output_file, bbox_inches="tight", pad_inches=0.02)
...
def process_impl(impl):
    ...
    # This workload has no --phase_boundaries, so parse_rocksdb_log returns a
    # single aggregate phase covering the whole run (100M inserts + 10K point
    # queries + 1K range queries).
    phasewise_wkl_ops = [100_011_000]
...
def plot_flush_and_compaction_counts():
    ...
    # Hash-based memtables flush/compact 10-30x more often than the rest
    # (~950-7.4K vs ~144K) -- a linear scale crushes the smaller ones to
    # near-invisible slivers, so use log scale to keep every bar legible.
    ax.set_yscale("log")
    ax.set_ylim(bottom=1)

    ax.set_ylabel("count", labelpad=-6)

    output_file = OUT_DIR / "disk-based-flush-and-compaction-counts.pdf"
    plt.savefig(output_file, bbox_inches="tight", pad_inches=0.02)
    ...
    legend_file = OUT_DIR / "disk-based-latency-legend.pdf"
    fig_legend.savefig(legend_file, bbox_inches="tight", pad_inches=0.02)
    plt.close(fig_legend)
    print(f"Saved legend to {legend_file}")
...
def plot_throughput():
    ...
    # --- center ticks under groups ---
    ax.set_xticks(x + bar_width * (num_impls - 1) / 2)
    ax.set_xticklabels(["overall"] if num_phases == 1 else [f"P{i+1}" for i in range(num_phases)])
    ...
    output_file = OUT_DIR / "disk-based-throughput.pdf"
    plt.savefig(output_file, bbox_inches="tight", pad_inches=0.02)
...
def plot_overall_datamovement():
    ...
    output_file = OUT_DIR / "disk-based-overall-datamovement.pdf"
...
def plot_write_stall():
    ...
    output_file = OUT_DIR / "disk-based-write-stall.pdf"
    plt.savefig(output_file, bbox_inches="tight", pad_inches=0.06)
    print(f"Saved plot to {output_file}")
...
def plot_block_read_count():
    ...
    labels = list(results.keys())
    num_impls = len(labels)
    # No --phase_boundaries for this workload -> a single aggregate phase
    # covering all point/range queries (PerfContext block_read_count is only
    # touched by the read path, so inserts don't pollute this).
    phases_to_show = [0]
    ...
    output_file = OUT_DIR / "disk-based-block-read-count.pdf"
    plt.savefig(output_file, bbox_inches="tight", pad_inches=0.06)
    print(f"Saved plot to {output_file}")
...
def plot_level_hits():
    ...
    output_file = OUT_DIR / "disk-based-level-hits-pq.pdf"
    plt.savefig(output_file, bbox_inches="tight", pad_inches=0.06)
    print(f"Saved plot to {output_file}")
...
if __name__ == "__main__":
    ...
    # Fast plots first (workload.log is tiny; PQ/RQ only match 10K/1K lines
    # in stats.log). Insert-latency parsing has to scan all ~100M "I:" lines
    # per implementation, so it's last -- see warning above.
    plot_PQ_latencies()
    plot_RQ_latencies()
    plot_throughput()
    ...
    # plot_insert_latencies() is dead code (superseded by plot_insert_latencies_violin above)
