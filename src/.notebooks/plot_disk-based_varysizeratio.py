...
# ==============================================================================
LOG_SCALE = False # Set to True for log scale, False for linear scale (global)
...
DATA_BASE_DIR = Path("/Users/cba/Desktop/LSM/LSMMemoryProfiling/data_new/diskbased-size-ratio")
PLOTS_DIR = Path("/Users/cba/Desktop/LSM/LSMMemoryProfiling/plots/vary_size_ratio")
...
def get_buffer_details(sub_dir_name):
    # Pattern to match the suffix -T(\d+)
    m = re.search(r"(.*)-T(\d+)$", sub_dir_name)
    if not m:
        return None, None
    ...
    # Mapping prefixes to the style keys in FILTER_BUFFERS
    mapping = {
        "hashlinkedlist-H100000-X6": "hash_linked_list-X6-H100000",
        "hashskiplist-H100000-X6": "hash_skip_list-X6-H100000",
        "hashvector-H100000-X6": "hash_vector-X6-H100000",
        "simpleskiplist": "simple_skiplist",
        "skiplist": "skiplist",
        "unsortedvector-preallocated": "unsortedvector-preallocated",
        "vector-preallocated": "vector-preallocated"
    }
    
    buffer_key = mapping.get(prefix, prefix)
    return ratio, buffer_key
...
def collect_records():
    ...
    for exp_dir in sorted(DATA_BASE_DIR.iterdir()):
        ...
        for buffer_dir in exp_dir.iterdir():
            ...
            ratio, buffer_key = get_buffer_details(buffer_dir.name)
            if ratio is None or buffer_key not in FILTER_BUFFERS: continue

            # Updated log filename from tree output
            log_path = buffer_dir / "workload.log"
...
def plot_lines(df, metric_col, y_label, filename_base, use_log, num_ticks, ylim, yticks):
    df_grouped = df.groupby(["size_ratio", "buffer_type"])[metric_col].mean().reset_index()
    df_grouped = df_grouped.sort_values("size_ratio")
    all_ratios = sorted(df_grouped["size_ratio"].unique())
    ratio_to_idx = {ratio: i for i, ratio in enumerate(all_ratios)}

    fig, ax = plt.subplots(figsize=(6, 5))
    for buf in df_grouped["buffer_type"].unique():
        buf_df = df_grouped[df_grouped["buffer_type"] == buf]
        style = line_styles.get(buf, {})
        plot_x = [ratio_to_idx[r] for r in buf_df["size_ratio"]]
