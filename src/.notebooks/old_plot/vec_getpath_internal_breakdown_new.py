...
X_AXIS_END_INDEX = 100001 # Updated for 100k data points

# --- Import plotting setup ---
try:
    from plot import *
    # Requirement: If specified font/style is not found, abort and return error.
    # (Assuming plot.py handles the font initialization as per your requirements)
except ImportError:
    # Requirement: If specified font/style is not found, abort and return error.
...
# Target: /Users/cba/Desktop/LSMMemoryBuffer/data
STATS_DIR = Path("/Users/cba/Desktop/LSMMemoryBuffer/data")
...
BUFFER_TO_PLOT = "buffer-2-vector-dynamic"
FIGSIZE = (5, 3)
NUM_RUNS = 1
...
def save_plot_legend(handles, labels, base_output_path):
    ...
    ext = "pdf"
    legend_fig.savefig(
        base_output_path.with_name(f"{base_output_path.name}_legend.{ext}"),
        bbox_inches="tight",
    )
...
def plot_component_latency(avg_data, output_filename):
    ...
    fig.subplots_adjust(
        left=0.15, right=0.98, top=0.98, bottom=0.15
    )  # Adjusted top margin since title is removed
    ...
    # Format x-ticks to use 'K' for thousands
    ...
    # Using bbox_inches="tight" and pad_inches as requested
...
def main():
    # Updated patterns for the new data naming convention
    patterns = {
        "sequential": r"sanitycheck-multiphase-.*",
        "interleave": r"sanitycheck-multiphase_interleave_vec_internal-.*",
    }
    ...
    for d in STATS_DIR.iterdir():
        ...
        run_data = []
        # UPDATED: Changed from stats.log to rocksdb_stats.log as per new tree structure
        log_file = buffer_dir / "rocksdb_stats.log"
        if log_file.exists():
        ...
        if not run_data:
            print(f"Error: {log_file.name} not found or empty in {buffer_dir}")
            continue
        ...
        filename_base = (
            PLOTS_DIR / f"latency_breakdown_{BUFFER_TO_PLOT}_{workload_type}"
        )
