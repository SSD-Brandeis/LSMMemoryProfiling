...
RAWOP_DIR = EXP_DIR / "filter_result_multiphase_sequential_inmemory_314"
...
TOKEN_PAT = {
    "insert": re.compile(r"-I-(\d+)"),
    "point": re.compile(r"-PQ-(\d+)"),
    "range": re.compile(r"-RQ-(\d+)"),
}
...
def collect_records(data_dir):
    ...
    for log_path in data_dir.rglob("workload*.log"):
        ...
        # Extract the buffer configuration name by dropping the "buffer-X-" prefix
        m_buf = re.match(r"^buffer-\d+-(.*)", buffer_name, re.IGNORECASE)
        if m_buf:
            raw_key = m_buf.group(1)
            # Map legacy buffer names to maintain existing plotting styles
...
def get_dynamic_caption(data_dir, suffix):
    if not data_dir.exists():
        return ""
    for path in data_dir.iterdir():
        if path.is_dir() and "sanitycheck" in path.name:
            m_i = re.search(r"-I-(\d+)", path.name)
            m_pq = re.search(r"-PQ-(\d+)", path.name)
            m_rq = re.search(r"-RQ-(\d+)", path.name)
            m_s = re.search(r"-S-([0-9.]+)", path.name)
            if m_i and m_pq and m_rq and m_s:
                i_val = int(m_i.group(1)) // 1000
                pq_val = int(m_pq.group(1)) // 1000
                rq_val = int(m_rq.group(1)) // 1000
                s_val = m_s.group(1)
                base = f"I={i_val}K PQ={pq_val}k RQ={rq_val}k S={s_val}"
                if suffix == "_rawop":
                    return f"Sequential {base}"
                elif suffix == "_interleave":
                    return f"Interleaved {base}"
                return base
    return ""
...
def process_and_plot(data_dir, file_suffix, y_limit_thr, y_limit_lat):
    ...
    caption_text = get_dynamic_caption(data_dir, file_suffix)
    if not caption_text:
        base_caption = "I=740K PQ=50k RQ=1k S=0.1"
