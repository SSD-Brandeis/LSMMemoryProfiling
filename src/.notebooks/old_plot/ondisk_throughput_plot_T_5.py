...
# Sequential directory (Updated to new path provided in tree command)
RAWOP_DIR = Path("/Users/cba/Desktop/LSMMemoryBuffer/data/filter_result_disk_T_5_1GB/fixed_rerun_ondisk_sequential_T_5_feb8-lowpri_true-I250000-U0-Q10000-S1000-Y0.1-T5-P1024-B32-E128")

# Interleave directory (Commented out as requested)
...
# PLOTS_DIR = Path(
#     "/Users/cba/Desktop/LSMMemoryBuffer/notebooks/paper_plot/multi_throughput_fixed_ondisk_rerun"
# )


PLOTS_DIR = Path(
    "/Users/cba/Desktop/LSMMemoryBuffer/notebooks/paper_plot/ondisk_throughput_plot_T_5_Feb8"
)
# -------------------------
...
def collect_records(data_dir):
    ...
    for log_path in data_dir.rglob("workload*.log"):
        ...
        # --- DEBUG PRINT BLOCK ---
        print(f"\n[DEBUG] File: {log_path.name} | Buffer: {buffer_key}")
        print(f"  Counts  -> Inserts: {n_ins}, PQ: {n_pq}, RQ: {n_rq}, Total: {n_total}")
        print(f"  Times   -> Inserts: {exec_ns['Inserts']}ns, PQ: {exec_ns['PointQuery']}ns, "
              f"RQ: {exec_ns['RangeQuery']}ns, Workload: {exec_ns['Workload']}ns")
        # -------------------------
...
def main():
    ...
    # --- COMMENTED OUT: Interleave Processing ---
    ...
    # Define placeholder success variable so check below doesn't crash if needed later
    success_interleave = False

    # --- COMMENTED OUT: Workload Comparison Block ---
    # The comparison logic requires both experiments to be successful.
