...
try:
    from plot import *
    # Based on your requirements, we must ensure the font is specifically loaded.
    # If plot.py exists but doesn't set the font, or if plot.py is missing, we abort.
    if 'matplotlib' in sys.modules:
        import matplotlib.pyplot as plt
    else:
        import matplotlib.pyplot as plt
except ImportError:
    # Requirement: If specified font/style is not found, abort and return error.
    print("Error: 'plot.py' or specified font styles not found. Aborting program.")
    sys.exit(1)

# Default line styles if not overridden by plot.py
line_styles = {
...
# Updated only the interleave path to the new rerun2_mar22 directory
EXPERIMENT_DIRS = {
    "interleave": Path(
        "/Users/cba/Desktop/LSMMemoryBuffer/data/multiphase_interleave_inmemory_rerun2_mar22/sanitycheck-multiphase_rawop_interleavererun2-I-7400000-U-0-PQ-100000-RQ-0-S-0.1-Low_Pri0/buffer-2-vector-dynamic"
    ),
    "rawop": Path(
        "/Users/cba/Desktop/LSMMemoryBuffer/data/filter_result_mar15_multiphase_rawop/sanitycheck-multiphase-I-740000-U-0-PQ-50000-RQ-1000-S-0.1-Low_Pri0/buffer-2-vector-dynamic"
    ),
}
...
def parse_stats_log(file_path: Path, label: str = ""):
    latencies = []
    print(f"--- [DEBUG] Parsing log file for {label}: {file_path}")
    ...
    try:
        with open(file_path, "r") as f:
            ...
            # Debug printout for count verification
            print(f"--- [DEBUG] {label} COUNT: {len(matches)} matches for 'Get Time'.")
...
def plot_get_latency():
    ...
    # Lowered 'top' to 0.85 to increase the margin above the plot area
    fig.subplots_adjust(left=0.18, right=0.95, top=0.85, bottom=0.20)
    ...
    for exp_key in experiment_keys:
        ...
        # Passing exp_key for clearer debug print
        latencies = parse_stats_log(log_file, exp_key)
    ...
    # --- Tick Control ---
    # Controlling x and y ticks explicitly.
    # Updated x-ticks to handle 100,000 queries and use K for thousands
    ax.set_xticks([0, 20000, 40000, 60000, 80000, 100000])
    ax.xaxis.set_major_formatter(ticker.FuncFormatter(lambda x, p: f'{int(x/1000)}K' if x != 0 else '0'))
    ax.set_xlabel("point queries number")
    
    # Y-axis ticks
    ax.set_yticks([0, 2e8, 4e8, 6e8])
    ...
    if max_y > 999:
        y_power = 8 # Fixed to 8 to match your data scale ($10^8$)
        ...
        ax.text(
            0.0,
            1.02, # Positioned above the frame
    ...
    caption_text = " same wl"

    if all_max_y: 
        for ext in ["pdf"]:
            output_path = base_out.with_suffix(f".{ext}")
            # Increased pad_inches to 0.1 for more margin around the tight bounding box
