...
void Stats::printStats() {
  ...
  for (int i = 0; i < files_in_tree; ++i)
    ...
    // std::cout << "file_id = " << fade_file_info[i].file_id << " level = " <<
    // fade_file_info[i].file_level
    //         << " entry_count = " << fade_file_info[i].file_entry_count << "
    //         tombstone_count = " << fade_file_info[i].file_tombstone_count
    //         << " file_size = " << fade_file_info[i].file_size << " file_age =
    //         " << fade_file_info[i].file_age
    //         << " file_ttl_exp = " << fade_file_info[i].file_ttl_expired <<
    //         "\n";

    // std::cout << "point_tombstones_in_tree = " << point_tombstones_in_tree <<
    // std::endl; std::cout << "updates_completed = " << updates_completed <<
    // std::endl; std::cout << "inserts_completed = " << inserts_completed <<
    // std::endl;
    update_proportion = (float)updates_completed / (float)inserts_completed;
  // std::cout << "update_proportion = " << update_proportion << std::endl;
  float superfluous_bytes =
      (float)point_tombstones_in_tree *
      ((key_size + 1)); // + (key_size + value_size)*(1 + update_proportion) );
  float unique_bytes =
      (inserts_completed - point_deletes_completed) * (key_size + value_size);
  ...
  std::cout << std::setfill(' ') << std::setw(l - 3) << space_amp;
  // std::cout << std::setfill(' ') << std::setw(l) <<
  // _env->delete_persistence_latency; // !YBS-feb15-XX!
  std::cout << std::setfill(' ') << std::setw(l + 7) << std::fixed
            << std::setprecision(2)                    // !YBS-sep09-XX!
