...
for line in Lines:
    ...
    if (
        prev_line_first_word == "cmpt_sty"
        or prev_line_first_word == "#p_ts_in_tree"
        or prev_line_first_word == "%space_amp"
    ):  # if previous line begins with one of these
        # if False:
    ...
    if current_line_first_word == "user_key_comparison_count":
        ...
        current_line_first_word == "thread_pool_id"
        or current_line_first_word == "files"
        or current_line_first_word == "Workload_parameters:"
    ):
        temp_removed_characters = re.findall(r"[-+]?\d*\.\d+|\d+", line)
        # temp_removed_characters = re.sub('[^0-9]',' ', line)
        # temp_list = temp_removed_characters.split()
