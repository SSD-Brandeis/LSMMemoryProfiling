...
R = REPO_ROOT / "data" / "memtable_scalability_vs_threads_ondisk"
OUT = R / "plots"
...
WRITE_MEMTABLES = ["skiplist", "simple_skiplist", "vector", "unsorted_vector",
                   "art", "tlx_btree"]
READ_MEMTABLES = WRITE_MEMTABLES

MIXED_MEMTABLES = ["skiplist", "simple_skiplist", "art", "tlx_btree",
                   "vector", "unsorted_vector", "sorted_vector"]
...
def load_read(path):
    """{memtable: {threads: ops/s}} -- no bg_jobs/unordered_write sweep for
    read/mixed (see run_memtable_scalability_ondisk.py's READ_DEFAULT_*/
    MIXED_DEFAULT_* constants), so results.csv has no those columns."""
    d = defaultdict(dict)
    for row in csv.DictReader(open(path)):
...
def plot_write_memtable(data, memtable, out_path):
    ...
    ax.set_title(f"on-disk 100\\% insert, {latex_escape(MEMTABLE_DISPLAY_NAMES.get(memtable, memtable))}: throughput")
...
def plot_write_all(data, uw, out_path):
    ...
    ax.set_title(f"on-disk 100\\% insert, cmw=1, uw={int(uw)}, bg=8: throughput")
...
def plot_read_memtable(data, memtable, config_str, out_path):
    ...
    ax.set_ylim(bottom=0)
    ax.set_title(f"on-disk 100\\% point query, {latex_escape(MEMTABLE_DISPLAY_NAMES.get(memtable, memtable))}: "
...
def main():
    ...
    parser.add_argument("--data-root", default=None,
                        help="Override the data root (default: "
                             "data/memtable_scalability_vs_threads_ondisk).")
    ...
    for stale in ["write_speedup.pdf", "write_throughput.pdf",
                  ...
                  "read_speedup.pdf", "read_throughput.pdf"]:
