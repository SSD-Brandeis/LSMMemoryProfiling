...
RAWOP_ROOT = Path("/home/cc/LSMMemoryProfiling/.result/raw_op")
...
for log_path in RAWOP_ROOT.rglob("workload.log"):
    ...
    if not m:
        print(f"[WARN] folder name unparsable: {struct_dir.name}")
        continue
...
def combined_bar(metric_cols, title, _ylabel, fname, log=False):
    ...
    for i, buf in enumerate(bufs):
        ...
        ax.bar(xs, vals, width=bar_w, edgecolor="black", facecolor="white",
               hatch=hatches[i % len(hatches)], lw=1, label=buf.replace("_"," "))
