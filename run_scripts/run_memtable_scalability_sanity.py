...
EXPERIMENT_ROOT = REPO_ROOT / "data" / "sanity_lowpri0_uw1"
WORKLOAD_SCRATCH = EXPERIMENT_ROOT / "_workload"
RUNNER_SCRIPT = "run_scripts/run_memtable_scalability_sanity.py"
...
#   mixed:    20_000 * (24+100) ~= 2.5 MB   (<1% of buffer, load+insert combined)
WRITE_OP_COUNT = 10000
READ_LOAD_OP_COUNT = 10000
READ_QUERY_OP_COUNT = 10000
# Must match concurrency_mixed_inmemory.spec.json's op_counts exactly
# (10_000 load + 10_000 insert + 10_000 point_query in the mixed section,
# i.e. a 50/50 insert/query split timed together).
MIXED_LOAD_OP_COUNT = 10000
MIXED_INSERT_OP_COUNT = 10000
MIXED_QUERY_OP_COUNT = 10000
...
def run_mixed_scenario(memtables, bg_jobs_list, unordered_write_list,
    ...
    workload_path = WORKLOAD_SCRATCH / "mixed_workload.txt"
    ...
    for name in memtables:
        ...
        for T in thread_counts:
            ...
            # right after DB::Open() (same mechanism as run_read_scenario),
            # so the loaded data never leaves this one memtable/process --
            # no restart, no flush needed. This phase is itself timed
            # (workload.log's [load phase] block), but excluded from
            # throughput.csv/ops_per_sec. The T shard
            # threads then replay the mixed (50% insert / 50% point_query,
            # interleaved) section, timed.
            cmd = [str(BIN_DIR / "working_version_mt"),
                  "--load_file", str(load_path),
                  "--threads", str(T), "-m", str(factory_id)
...
def write_manifest(scenario, bg_jobs_list, unordered_write_list,
    ...
    manifest["provenance"] = {
        ...
        "command_setup": {
            "write": "python3 run_scripts/run_memtable_scalability_sanity.py --scenario write",
            "read": "python3 run_scripts/run_memtable_scalability_sanity.py --scenario read",
            "mixed": "python3 run_scripts/run_memtable_scalability_sanity.py",
...
def main():
    ...
    parser.add_argument("--scenario", choices=["write", "read", "mixed"], default="mixed",
                        help="Run exactly one scenario (run one at a time). "
                             "Defaults to 'mixed' so no CLI argument is needed.")
