...
Same 128 MB buffer / 6,000,000-op "winning combo" as ondisk_skiplist_diskio
(bg_jobs=8, mwb=16) -- unlike run_ondisk_subcompactions_sweep_skiplist.py's
smaller 8 MB / 450,000-op config, which turned out inconclusive (compaction
jobs too small/brief at that scale to show a clean max_subcompactions
signal). This scale gives compaction real, sustained work to split across
threads, and is directly comparable to the ~236 MB/s figure this test is
trying to explain.
"""
...
EXPERIMENT_ROOT = REPO_ROOT / "data" / "ondisk_subcompactions_sweep_skiplist_128mb"
WORKLOAD_SCRATCH = EXPERIMENT_ROOT / "_workload"
RUNNER_SCRIPT = "run_scripts/run_ondisk_subcompactions_sweep_skiplist_128mb.py"
PLOT_SCRIPT = "plot_scripts/plot_ondisk_subcompactions_sweep_skiplist_128mb.py"

THREAD_COUNTS = [1, 2, 4, 8, 16]
MEMTABLE_NAME = "skiplist"
MEMTABLE_FACTORY_ID = 1

# Same geometry as ondisk_skiplist_diskio / ondisk_bottleneck_sweep_all_
# memtables (see AGENTS.md "Buffer Geometry"): E = real entry size, B*E =
# 4096B page, P derived so P*B*E is exact.
...
WRITE_SPEC = EXPERIMENT_ROOT / "write_100pct" / "concurrency_write_subcompactions_sweep_128mb.spec.json"
...
def write_manifest(thread_counts):
    manifest = {
        ...
        "note": "128 MB / 6,000,000-op rerun of "
            "run_ondisk_subcompactions_sweep_skiplist.py's 8 MB / "
            "450,000-op sweep, which was inconclusive (compaction jobs "
            "too small/brief at that scale). Same winning combo (bg_jobs="
            "8, mwb=16, buffer=128MB) as ondisk_skiplist_diskio, so results "
            "are directly comparable to that experiment's ~236 MB/s peak "
            "measured disk write throughput and disk_raw_benchmark's "
            "~430 MB/s raw sustained dd throughput to the same device.",
        "harness": {
        ...
        "provenance": {
            ...
            "command_setup": {
                "write": "python3 run_scripts/run_ondisk_subcompactions_sweep_skiplist_128mb.py",
                "plot": "python3 plot_scripts/plot_ondisk_subcompactions_sweep_skiplist_128mb.py",
            },
            "metric_definitions": {
