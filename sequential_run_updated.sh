...
reorder_workload () {
  local f="$1"
  tmpI=$(mktemp); tmpQ=$(mktemp); tmpS=$(mktemp)
  grep '^I ' "$f" >"$tmpI" || true
  grep '^Q ' "$f" >"$tmpQ" || true
  grep '^S ' "$f" >"$tmpS" || true
...
for i in "${!page_sizes[@]}"; do
  ...
  if   [ "${PAGE_SIZE}" -eq 2048 ];  then PAGE_TAG="2kb_page"  ; PAGES_PER_FILE_LIST=(4096)
  elif [ "${PAGE_SIZE}" -eq 4096 ];  then PAGE_TAG="4kb_page"  ; PAGES_PER_FILE_LIST=(16384)
  elif [ "${PAGE_SIZE}" -eq 8192 ];  then PAGE_TAG="8kb_page"  ; PAGES_PER_FILE_LIST=(1024)
  elif [ "${PAGE_SIZE}" -eq 16384 ]; then PAGE_TAG="16kb_page" ; PAGES_PER_FILE_LIST=(512)
  elif [ "${PAGE_SIZE}" -eq 32768 ]; then PAGE_TAG="32kb_page" ; PAGES_PER_FILE_LIST=(256)
  elif [ "${PAGE_SIZE}" -eq 65536 ]; then PAGE_TAG="64kb_page" ; PAGES_PER_FILE_LIST=(128)
